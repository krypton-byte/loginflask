...
def verif(email,password):
	...
	op=nn.execute('SELECT * FROM LOGIN WHERE EMAIL="%s" AND PASSWORD="%s"'%(email,password))
	try:
		for x in op:
			if x[1] == email and x[2] == password:
				inter='NAMA : %s<br>EMAIL : %s <br>PASSWORD : %s<br>ALAMAT : %s<br>GENDER : %s<br>SEKOLAH : %s<br>NO-WA : %s<br>UMUR : %s<br>STATUS : %s'%(escape(x[0]),escape(x[1]),escape(x[2]),escape(x[3]),escape(x[4]),escape(x[5]),escape(x[6]),escape(x[7]),escape(x[8]))
